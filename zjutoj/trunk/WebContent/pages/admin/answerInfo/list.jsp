...
<script type="text/javascript">
	function getPageData(pageNo) {
		...
		location.href = path + '/study/answerInfoAction.do?method=mlist&pageNo='+pageNo+'&searchType='+searchType;
	}
	
	function detail(id) {
		location.href = path + '/study/answerInfoAction.do?method=mdetail&answerId='+id;
	}
	
	function save() {
		location.href = path + '/study/answerInfoAction.do?method=msaveUI';
	}
	
	function searchList() {
		var type = $('#search_type').val();
		location.href = path + '/study/answerInfoAction.do?method=mlist&searchType='+type;
	}
</script>
</head>
<body>
			...
			<ul>
				<li>
					...
					<select id="search_type">
						...
						<option value="0" <s:property value="%{(#searchType == 0) ? 'selected' : ''}" />>未审核</option>
						<option value="1" <s:property value="%{(#searchType == 1) ? 'selected' : ''}" />>审核</option>
					</select>
				</li>
